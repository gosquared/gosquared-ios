...
@implementation NSString (Emojione)
...
- (NSString *)stringWithResolvedShortnameCodes
{
    ...
    dispatch_once(&onceToken, ^{
        emojiMapping = [NSString _loadShortNameToUnicode];
    });

    NSString * SHORTNAME_PATTERN = @":([-+\\w]+):";

    NSMutableString * unicodeString = [self mutableCopy];
    NSError * error = nil;
    ...
    for (NSTextCheckingResult * result in [matches reverseObjectEnumerator]) {
        ...
        NSString * shortname = [self substringWithRange:[result rangeAtIndex:1]];
